...
byte    opposite[9] = {west,southwest,south,southeast,east,northeast,north,northwest,nodir};
...
void    SpawnNewObj (byte tilex, byte tiley, statetype *state);
...
void SpawnNewObj (byte tilex, byte tiley, statetype *state)
{
    ...
    if (state->tictime)
        newobj->ticcount = (US_RndT() % state->tictime) + 1;
    else
        newobj->ticcount = 0;
...
void KillActor (objtype *ob)
{
    ...
    tilex = (byte)(ob->x >> TILESHIFT);         // drop item on center
    tiley = (byte)(ob->y >> TILESHIFT);
...
//
    ...
    // Aryan: make actor active at this point?
