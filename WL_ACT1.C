...
void DoorClosing (int door)
{
    ...
    if (((unsigned)actorat[tilex][tiley] != (door | 0x80)) || (player->tilex == tilex && player->tiley == tiley))
    {
        //
...
int  pwalldirs[4][2] = { {0,-1},{1,0},{0,1},{-1,0} };
...
void PushWall (int checkx, int checky, int dir)
{
    ...
    //
    // set correct floorcode in the now empty spot
    //
    map = &MAPSPOT(pwallx,pwally,0);
    tile = *map;
...
void MovePWalls (void)
{
    ...
    if (pwallstate >> 7 != oldblock)
    {
        ...
        dx = pwalldirs[pwalldir][0];
        dy = pwalldirs[pwalldir][1];
