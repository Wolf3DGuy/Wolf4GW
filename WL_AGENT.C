...
void StatusDrawPic (unsigned x, unsigned y, unsigned picnum)
{
    ...
    bufferofs = (byte *)(SCREENSEG + PAGE1START+(200-STATUSLINES)*SCREENWIDTH);
    LatchDrawPic (x,y,picnum);
    bufferofs = (byte *)(SCREENSEG + PAGE2START+(200-STATUSLINES)*SCREENWIDTH);
    LatchDrawPic (x,y,picnum);
    bufferofs = (byte *)(SCREENSEG + PAGE3START+(200-STATUSLINES)*SCREENWIDTH);
    LatchDrawPic (x,y,picnum);
...
void TakeDamage (int points,objtype *attacker)
{
    ...
    //
    #ifdef SPEAR
    if (points > 30 && gamestate.health && !godmode)
    {
...
void ClipMove (objtype *ob, long xmove, long ymove)
{
    ...
    && ob->x < (((fixed)(mapwidth - 1)) << TILESHIFT)
    && ob->y < (((fixed)(mapheight - 1)) << TILESHIFT))
...
void KnifeAttack (objtype *ob)
{
    ...
    for (check = ob->next; check; check = check->next)
    {
        ...
        {
            if (abs(check->viewx - centerx) < shootdelta && check->transx < dist)
            {
                dist = check->transx;
                closest = check;
            }
...
void VictorySpin (void)
{
    ...
    desty = (((fixed)player->tiley - 5) << TILESHIFT) - 0x3000;
