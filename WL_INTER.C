...
void LevelCompleted (void)
{
    ...
    int x,y,i,min,sec,ratio,kr,sr,tr;
    byte *temp;
    char tempstr[10];
...
#ifndef SPEAR
    if (mapon<8)    // TODO: use macros for this
#else
    if (mapon != 4 &&
        mapon != 9 &&
        mapon != 15 &&
...
#define PRINTCOLOR      HIGHLIGHT
    ...
    int i,match,whichboss,bossnum,attempt,whichline,start;
    ...
    while (attempt<3)
    {
        ...
        switch(quiztype)
        {
            ...
            case debriefing:
                ...
                {
                    if (!_fstrnicmp(inputbuffer,bossstrs[whichboss]+i,strlen(inputbuffer)) && strlen(inputbuffer)>3)
                        match = 1;
                }
            ...
            case staffquiz:
                ...
                {
                    if (!_fstrnicmp(inputbuffer,MemberCorrect[whichmem]+i,strlen(inputbuffer)) && strlen(inputbuffer)>2)
                        match = 1;
                }
                match += BackDoor(inputbuffer);
                break;
