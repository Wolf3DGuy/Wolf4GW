...
{
    char        wordtable[WORDLIMIT];
    int         wordindex;
    word        wwidth,wheight,newpos;


    //
    // copy the next word into [wordtable]
    //
    wordtable[0] = *text++;
    wordindex = 1;
    while (*text>32)
    {
        wordtable[wordindex] = *text++;
        if (++wordindex == WORDLIMIT)
            Quit ("PageLayout: Word limit exceeded");
    }
    wordtable[wordindex] = 0;        // stick a null at end for C

    //
    // see if it fits on this line
    //
    VW_MeasurePropString (wordtable,&wwidth,&wheight);
...
#endif
{
    ...
    boolean     newpage,firstpage,done;
    ScanCode    scan;
