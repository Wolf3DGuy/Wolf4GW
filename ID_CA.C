...
byte        grstarts[(NUMCHUNKS+1)*3]; // array of offsets in vgagraph, -1 for sparse
...
void CAL_HuffExpand(byte *source, byte *dest, long length, huffnode *hufftable, boolean screenhack)
{
    byte     *end;
    huffnode *headptr;
    byte     mapmask;
...
//
    ...
    tinf = (mapfiletype *)malloc(sizeof(mapfiletype));
    read(handle,tinf,sizeof(mapfiletype));
    close(handle);
...
//
    ...
    if ((maphandle = open(fname,O_RDONLY | O_BINARY, S_IREAD)) == -1)
        CA_CannotOpen(fname);
#else
...
//
    for (i=0;i<MAPPLANES;i++)
        mapsegs[i]=(word *)malloc(MAPAREA * sizeof(word));
}
...
// Sprites need to have shifts made and various other junk
//
    CAL_HuffExpand (source,bufferofs,expanded,grhuffman,true);
...
void CA_CacheMap (int mapnum)
{
    ...
    if (mapheaderseg[mapnum]->width != MAPSIZE || mapheaderseg[mapnum]->height != MAPSIZE)
        Quit ("Map not %u*%u!",MAPSIZE,MAPSIZE);
...
//
    ...
    for (plane = 0; plane < MAPPLANES; plane++)
    {
        ...
        CA_RLEWexpand (((word *)buffer2seg)+1,(word *) *dest,size,tinf->RLEWtag);
        free(buffer2seg);
