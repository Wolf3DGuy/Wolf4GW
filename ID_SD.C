...
#define SDL_SoundFinished() {SoundNumber = 0; SoundPriority = 0;}
...
static longword SDL_SBPlaySegInIRQ(volatile byte *data,longword length)
{
    ...
    bufoffs=dataofs-(word)DMABuffer;
    lenleft=8192-bufoffs;
...
SDL_StartSB(void)
{
    byte    timevalue,test;
    byte    *buffer;
    int     datapage,dataofs;

    sbIntVec = sbIntVectors[sbInterrupt];
    if (sbIntVec < 0)
        Quit ("SDL_StartSB: Illegal or unsupported interrupt number for SoundBlaster");

    if (DPMI_GetDOSMemory((void **)&buffer,&DMABufferDescriptor,4*DMABUFFERSIZE))
            Quit ("SDL_StartSB: Unable to allocate DMA buffer");

    if (((longword)buffer & 0xffff) + (2*DMABUFFERSIZE) > 0x10000)
            buffer = (byte *)(((longword) buffer & 0xff0000) + 0x10000);
    ...
    datapage = ((int)DMABuffer >> 16) & 255;
    dataofs = (int)DMABuffer;
...
SDL_CheckSS(void)
{
    ...
    if (inp(ssStatus) & 0x40)
        goto checkdone;	     // Check to see if FIFO is currently empty
...
outloop:
    ...
    if (inp(ssStatus) & 0x40)
        present = true;      // Is FIFO overflowed now?
                ...
                while (*env)
                {
                    ...
                    switch (toupper(*env))
                    {
                        ...
                        case 'I':
                            ...
                            if ((temp >= 0) && (temp <= 10) && (sbIntVectors[temp] != -1))
                            {
                                sbInterrupt = temp;
                                sbIntVec = sbIntVectors[sbInterrupt];
                            }
