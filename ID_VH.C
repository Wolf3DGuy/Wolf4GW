...
void VH_UpdateScreen (void)
{
	...
	updateptr = &update[0][0];
	blockstart = &blockstarts[0];

	for (y = 0; y < UPDATEHIGH; y++)
	{
		for (x = 0; x < UPDATEWIDE; x++, updateptr++, blockstart++)
		{
			if (*updateptr)
			{
				*updateptr = 0;
				...
				{
					*(displayofs + offs) = *(bufferofs + offs);
					*(displayofs + offs + 1) = *(bufferofs + offs + 1);
					*(displayofs + offs + 2) = *(bufferofs + offs + 2);
					*(displayofs + offs + 3) = *(bufferofs + offs + 3);
				}
			}
		}
	}
...
int VW_MarkUpdateBlock (int x1, int y1, int x2, int y2)
{
	...
	mark = (byte *)update + uwidthtable[yt1] + xt1;
...
void LatchDrawPic (unsigned x, unsigned y, unsigned picnum)
{
	...
	VL_LatchToScreen (source,wide >> 2,height,x << 3,y);
}
...
boolean FizzleFade (unsigned source, unsigned dest,	unsigned width, unsigned height, unsigned frames, boolean abortable)
{
	...
	do
	{
		...
		for (p=0;p<pixperframe;p++)
		{
			...
			*(byte *)(SCREENSEG + drawofs + pagedelta) = *(byte *)(SCREENSEG + drawofs);
