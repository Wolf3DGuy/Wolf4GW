...
statetype s_spectredie1 = {false,SPR_SPECTRE_F1,10,NULL,A_DeathScream,&s_spectredie2};
...
void A_Dormant (objtype *ob)
{
    ...
    ob->flags &= ~(FL_ATTACKMODE | FL_NONMARK);
    ob->dir = nodir;
...
void T_Chase (objtype *ob)
{
    ...
    if (CheckLine(ob))  // got a shot at player?
    {
        ...
        if (!dist || dist == 1)
        {
            ...
            if (deltax <= 0x14000l && deltay <= 0x14000l)
                chance = 300;
        }
        else
            chance = (tics << 4) / dist;
...
void T_DogChase (objtype *ob)
{
    ...
    //
            dx = LABS(player->x - ob->x) - move;
            dy = LABS(player->y - ob->y) - move;
...
void T_Path (objtype *ob)
{
    ...
    while (move)
    {
        ...
        if (!ob->tilex || ob->tilex > (mapwidth - 1) || !ob->tiley || ob->tiley > (mapheight - 1))
        {
...
void T_Bite (objtype *ob)
{
    ...
    dx = LABS(player->x - ob->x) - TILEGLOBAL;
    dy = LABS(player->y - ob->y) - TILEGLOBAL;
...
void A_StartDeathCam (objtype *ob)
{
    ...
    FizzleFade ((long)bufferofs - SCREENSEG,(long)displayofs - SCREENSEG,320,200 - STATUSLINES,70,false);
...
//
    ...
    for (i = 0; i < 3; i++)
    {
        bufferofs = (byte *)(SCREENSEG + screenloc[i]);
        DrawPlayBorder ();
    }
    bufferofs = temp;
