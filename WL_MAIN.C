...
boolean SaveTheGame (int file, int x, int y)
{
    ...
    for (j = 0; j < mapheight; j++)
    {
        for (i = 0; i < mapwidth; i++)
        {
            ...
            if ((long)check & 0xffff0000)
                actnum = 0x8000 | (word)(check - objlist);
            else
                actnum = (word)check;
...
boolean LoadTheGame (int file, int x, int y)
{
    ...
    while (1)
    {
        ...
        memcpy (newobj,&nullobj,sizeof(nullobj) - 8);    // don't copy over the links
    }
    ...
    if (gamestate.secretcount)
    {
        ...
        for (j = 0; j < mapheight; j++)
        {
            for (i = 0; i < mapwidth; i++)
            {
                ...
                if (icon == PUSHABLETILE && !tilemap[i][j] && (tile < AREATILE || tile >= (AREATILE + NUMAREAS)))
                {
                    if (*map1 >= AREATILE)
                        tile = *map1;
                    if (*(map1 - 1 - mapwidth) >= AREATILE)
                        tile = *(map1 - 1 - mapwidth);
                    if (*(map1 - 1 + mapwidth) >= AREATILE)
                        tile = *(map1 - 1 + mapwidth);
                    if (*(map1 - 2) >= AREATILE)
                        tile = *(map1 - 2);
...
//
    ...
    sintable[ANGLEQUAD] = GLOBAL1;
    sintable[VANG270] = -GLOBAL1;
}
...
{
#ifndef SPEAR
    VW_Bar (0,189,300,11,*(byte *)SCREENSEG);
    WindowX = 0;
    WindowW = 320;
    PrintY = 190;
...
//
	...
	for (y = 0; y < UPDATEHIGH; y++)
	{
		for (x = 0; x < UPDATEWIDE; x++)
			*blockstart++ = ylookup[y * 16] + (x * TILEWIDTH);
    }
...
//
    ...
    displayofs = (byte *)(SCREENSEG  + PAGE1START);
    bufferofs = (byte *)(SCREENSEG + PAGE2START);
}
